  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        >
          <button onClick={onClose} className="absolute top-4 right-4 z-10 p-2 rounded-lg bg-white/80 dark:bg-slate-800/80 backdrop-blur hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
          ...
          <div className="p-6 space-y-6">
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400 mt-2 leading-relaxed">{project.description}</p>
            </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {project.techStack.map((t) => (
                    <span key={t} className="px-2.5 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-xs font-medium text-slate-600 dark:text-slate-400">{t}</span>
                  ))}
                </div>
              </div>
            )}
            ...
            <div>
                <h4 className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">Completion</h4>
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold text-slate-900 dark:text-slate-100 tabular-nums">{project.completion || 0}%</span>
              </div>
            </div>
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {project.challenges.map((c) => (
                    <li key={c} className="text-sm text-slate-500 dark:text-slate-400 pl-4 relative">
                      <span className="absolute left-0 top-2 w-1.5 h-1.5 rounded-full bg-amber-400 dark:bg-amber-500" />
                      {c}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                ...
                <ul className="space-y-1.5">
                  {project.futureImprovements.map((fi) => (
                    <li key={fi} className="text-sm text-slate-500 dark:text-slate-400 pl-4 relative">
                      <span className="absolute left-0 top-2 w-1.5 h-1.5 rounded-full bg-blue-400 dark:bg-blue-500" />
                      {fi}
                    </li>
                  ))}
                </ul>
            ...
            {project.milestones?.length > 0 && (
              <div>
                <h4 className="text-xs font-semibold text-slate-700 dark:text-slate-300 uppercase tracking-wider mb-2">Timeline</h4>
                <div className="space-y-0">
                  {project.milestones.map((m, i) => (
                    <div key={i} className="flex gap-3">
                      <div className="flex flex-col items-center">
                        <div className={`w-3 h-3 rounded-full shrink-0 mt-1 ${i === project.milestones.length - 1 ? 'bg-emerald-500 ring-2 ring-emerald-200 dark:ring-emerald-800' : 'bg-slate-300 dark:bg-slate-600'}`} />
                        {i < project.milestones.length - 1 && <div className="w-px flex-1 bg-slate-200 dark:bg-slate-700 my-1" />}
                      </div>
                      <div className="pb-4">
                        <p className="text-sm font-medium text-slate-700 dark:text-slate-300">{m.label}</p>
                        <p className="text-[11px] text-slate-400 dark:text-slate-500">{m.date}</p>
                      </div>
                    </div>
                  ))}
                </div>
